...
namespace Oxide.Plugins
{
    ...
    internal class VehicleDeployedLocks : CovalencePlugin
    {
        ...
        private void OnServerInitialized()
        {
            ...
            _lockedVehicleTracker.OnServerInitialized(this);
            _lockExpirationManager.OnServerInitialized(this, _lockedVehicleTracker);
        ...
        private class LockedVehicleTracker
        {
            ...
            public void OnServerInitialized(VehicleDeployedLocks pluginInstance)
            {
                ...
                foreach (var entity in BaseNetworkable.serverEntities)
                {
                    ...
                    var vehicleInfo = pluginInstance._vehicleInfoManager.GetVehicleInfo(baseEntity);
            ...
            {
                var vehicleInfo = _pluginInstance._vehicleInfoManager.GetVehicleInfo(entity);
                if (vehicleInfo == null)
                    return null;
        ...
        {
            public void OnServerInitialized(VehicleDeployedLocks pluginInstance, LockedVehicleTracker lockedVehicleTracker)
            {
                var settings = pluginInstance._pluginConfig.AutoUnlockSettings;

                if (!settings.Enabled)
                    return;

                pluginInstance.timer.Every(settings.CheckIntervalSeconds, () =>
                {
                    List<BaseEntity> remainingVehicles = null;

                    foreach (var entry in lockedVehicleTracker.VehiclesWithLocksByType)
                    {
                        ...
                        foreach (var vehicle in vehicleList)
                        {
                            ...
                            if (settings.ExemptOwnedVehicles && vehicle.OwnerID != 0)
                                continue;
                            ...
                            if (timeSinceUsed < settings.IdleSeconds)
                                continue;
                    ...
                    if (remainingVehicles != null)
                    {
                        ...
                        pluginInstance.timer.Repeat(0, remainingVehicles.Count, () =>
                        {
